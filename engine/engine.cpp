...
void Engine::removeThing(Thing *thing) {
    ...
    deletableThings.push_back(thing);
}
...
void Engine::clickEvent(int button, int action) {
    ...
    for (const std::unique_ptr<Thing> &thing : things)
        thing->click(buttonCode, actionCode);
}
...
void Engine::loop() {
    while (!glfwWindowShouldClose(window)) {
        ...
        for (Thing *toDelete : deletableThings) {
            for (size_t a = 0; a < things.size(); a++) {
                if (things[a].get() == toDelete) {
                    things.erase(things.begin() + a);
                    break;
                }
            }
        }

        deletableThings.clear();
